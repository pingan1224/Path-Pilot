...
### flaky 签名:诊断有效,修复被回退(2026-08-17)

三个签名(deferred 而非 answered、缺 policy 引用、跳过 `get_course_info`)是 2026-08-17
那次干净 gate 才浮出来的形状:签名稳定、成员在跑与跑之间流动。做了一轮修复,**gate 变红,
已整体回退。** 留下的是诊断,不是补丁。
...
公开政策供不出这两条,它们是逐课的。失败那两次是**替换**而非遗漏:模型改调 `search_policy`,
然后在不知道决定性事实的情况下回答了。**所以是模型错,不是测试错。**

第二条诊断:`albert_checklist` 返回值里的 `instruction` 与系统提示词矛盾。它只说「说你看不到、
指路、别猜」,一个字没提「规则那一半仍然要答、并引用 `policy:chunk`」——而系统提示词把这两件事
都写对了。**提示词不是杠杆,它已经最大程度明确并且在输**:工具返回值在模型刚调完工具那一刻抵达,
比系统提示词近,矛盾时它赢。

**回退的原因是方法,不是方向。** 一次改了两处(工具 instruction + `get_course_info` 描述),
结果:

| | 22:48 基线 | 带改动 |
|---|---|---|
| gate | PASS | **FAIL —— forbidden tool calls 2** |
| B35(单词 "help") | 未开火 | **同时调 `propose_mission_candidates` 和 `start_mission`** |
| calls/run | 1.58 | 1.81 |
| B12 / B29 | flaky | improved |

forbidden 在此前四次跑里都是 0(12 次 B35 尝试)。**改动整体把模型推得更主动**,而 B12/B29
的改善与红灯来自同一次跑,**无法归因到具体哪一处**——这就是一次改两样的代价。

收窄版(只退掉那句重复系统提示词、且唯一往「行动」推的话)在子集上 forbidden 回到 0,但只有
3 次 B35 尝试,与「修好了」和「运气好」都相容,没有采信。

**下次要动这三个签名,规矩是:一次只改一处,每处配一次全量 `--gate --repeat 3`。** 子集
证明不了任何事——这一天里每一个子集结论最后都被全量推翻,包括 B02 的 1/3→3/3。
