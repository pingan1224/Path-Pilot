...
Path Pilot 是一个面向 NYU School of Professional Studies（SPS）研究生的 **AI 选课规划智能体**。学生通过对话描述目标、粘贴注册错误或导入非官方成绩单，系统读取学生主动提供的课程记录，结合公开发布的项目要求、课程目录和学校政策，**生成下一学期的选课方案**——每一门都带上"为什么是它、为什么是现在"，并说明这个排法对毕业时间意味着什么。

产品一句话（2026-08-13 修订）：

> **Path Pilot 是帮助学生规划下一学期课程、并持续保持毕业进度的 AI 智能体。**

这句话替换了此前的"我现在能注册吗、还能不能按时毕业"。差别不是措辞：旧问题要的是一个**状态**（是/否），新问题要的是一个**交付物**（一份课表）。"能否按时毕业"没有被删掉，而是**从独立承诺变成规划时的约束与结果**——它不再作为二元判断出现，而是附着在每一门课上：这个排法是否保住按时毕业的路径、哪门课不修会造成延迟、还有哪些组合可选及各自风险。二元判断一旦错就整个错，而"少修这门推迟一学期"即使偏了学生也能自行复核。

Path Pilot 不连接 Albert，不读取官方学生记录，不代表 NYU，也不执行注册、清除 hold、批准例外或修改官方成绩。**它也不再读取 hold 状态**（见 §4.4）。它的产品承诺是：

> 告诉 Path Pilot 你修过什么、正在修什么以及想完成什么；Path Pilot 会给出下学期该修哪几门、每一门的理由和代价，并把不能确认的部分明确留给你或学校工作人员决定。
...
成为 SPS 学生每学期选课时最可信的独立规划层：基于学生自报的已修课程、专业要求、先修规则、课程负荷偏好与目标毕业时间，给出下一学期最合适的选课方案，并把真正需要 Albert 或学校工作人员确认的问题清楚地交接出去。

**"课程负荷偏好"指每学期修几门、哪些学期不修，不指上课时段。** 课程目录只发布开课学期（`typically_offered`），不含上课时间和 section，所以本产品无法处理"只上晚课"这类需求，也不得暗示自己可以。这是数据边界，不是待办。
...
- **给出下一学期的选课方案，每一门都带理由和代价**——为什么是它、为什么是现在、不修会推迟多久；
...
- **为每门候选课计算“反事实延迟代价”**：把它推后一个学期重排，看完成学期是否移动。这是把“课程清单”变成“有理由的方案”的关键一步，目前不存在——现有能力只能在**排不出来**时说明哪条约束卡住，不能在**可行**方案上归因；
- **新增“下学期方案”卡片类型**：现有 Artifact 词表只有 `mission_state / course_sequence / decoder_result`，装不下“每门带理由 + 延迟代价 + 备选组合”；
- 统一两处互相矛盾的每学期学分上限假设（`services/readiness.py` 用 12，`sequence/plan.py` 用 9）。旧定义下两条路各说各话无人察觉；新定义下毕业时间是规划的核心约束，同一学生问两次会得到两个不同的完成学期；
...
- **不读取、不展示、不主张任何 hold 状态**（见 §4.4）；
- 不在当前阶段支持 NYU 全部学院和全部项目的高置信 degree audit。

### 4.4 hold 的降级：一次记录在案的取舍（2026-08-13）

源 RFP 把 hold 处理列为核心场景之一，并承诺 *90% escalation accuracy for high-stakes cases*。本次修订**主动放弃了其中"读取 hold 状态"这一部分**，理由记录如下，以免日后被读成遗漏：

**没有 Albert 权限，产品能显示的 hold 只可能来自我们自己造的 fixture。** 全产品的诚实原则是"没有访问能力就说没有访问能力，而不是返回空结果"——holds 是唯一一处，产品用与计算结果相同的置信度，展示了一条关于学生官方记录的虚构事实。在选课规划这个定义下它也不在关键路径上:hold 挡的是"点注册"这一下，不是"该选哪几门"这个决定。

**一个必须补上的限定语(2026-08-13 实现时才发现):上面这句话此前只对 demo 成立。** 代码里早就存在 live/demo 两套工具面——真实用户从来看不到 `get_holds`，live mode 用 `albert_checklist` 告诉他们去哪儿查，注释写的正是"模型调不到的工具，就是模型做不出的主张"。也就是说,诚实版设计一直都在，读 fixture 的那套是**只有演示模式在用**。

这不会让问题变小,只是换了个位置:**演示模式才是任何人评判这个项目时打开的东西**,而它比真实产品能力更强,多出来的那部分全是编的数据。所以最终的做法不是"给 holds 设计一套降级方案"——那套方案已经存在并且已在跑——而是**删掉另一条岔路,让 demo 和真实用户拿到同一个产品**。

**升级规则不变。** hold 相关问题仍属高风险、仍然升级给人工。改变的是**这条主张从哪来**，只保留两种诚实形态，没有第三种：

1. **静态注册前提示**——"注册窗口打开前去 Albert 确认有没有 hold",不对该学生主张任何事实；
2. **学生自述**——学生自己说有，或粘贴报错文本交给 `decode_registration_error` 解析。那是自报输入，与成绩单同一性质，不需要任何 Albert 权限。decoder 拒绝把 hold code 映射到具体办公室这一条继续有效：本项目从未见过学校的 hold code 表。

**产品永远不得说"你有一个 hold"，只可以说 hold 会造成什么、该找谁。**
...
> 下学期我该选哪几门？这样排还能不能按时毕业？有哪些风险需要在 Albert 或 advisor 那里确认？
